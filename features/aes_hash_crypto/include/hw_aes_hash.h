 ...
 * \deprecated Consider using hw_aes_hash_is_key_revoked instead
 *
 */
DEPRECATED_MSG("Consider using hw_aes_hash_is_key_revoked instead")
 ...
 *
 * \return key memory address or 0 if key has been revoked
 * \deprecated Consider using hw_aes_hash_keys_address_get instead
 *
 */
DEPRECATED_MSG("Consider using hw_aes_hash_keys_address_get instead")
 ...
 * alternative version of execution flow is used, which may use DMA channel or simple OTP read.
 *
 * \note
 * After an AES operation with a certain key, further operations using the same key do not
 * need storing it to the AES/HASH memory again. If Secure DMA option is selected then
 ...
 *
 * \param[in] key_size The size of the key used in the encryption/decryption
 * \param[in] aes_keys The otp address or the buffer which contains the keys
 * \param[in] key_exp Define if key expansion will be performed by hw or sw
 *
 * \deprecated Consider using hw_aes_hash_otp_keys_load or hw_aes_hash_keys_load instead
 *
 */
DEPRECATED_MSG("Consider using hw_aes_hash_otp_keys_load or hw_aes_hash_keys_load instead")
void hw_aes_hash_store_keys(hw_aes_key_size key_size, const uint8 *aes_keys,
                            hw_aes_hash_key_exp_t key_exp);

/**
 * \brief Loads the keys from OTP memory to crypto engine
 *
 * This function loads the encryption/decryption keys from OTP memory to crypto engine
 * and performs key expansion by using the crypto engine. Moreover, it checks whether the
 * secure DMA channel is enabled and if this is the case it transfers the keys by using it.
 * Otherwise, it performs a direct OTP memory reading.
 *
 * \param[in] key_size The size of encryption/decryption key
 * \param[in] otp_keys_addr The otp address which contains the keys
 */
void hw_aes_hash_otp_keys_load(hw_aes_key_size key_size, const uint8 *otp_keys_addr);
